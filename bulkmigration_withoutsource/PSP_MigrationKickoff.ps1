...
<#
.DESCRIPTION
    PowerSyncPro Migration Kickoff Script (1:1 user migration)

    - Supports Local, AD, and Entra source account types.
    - Supports a single 1-to-1 mapping: one source user to one target user.
    ...
    username format depends on source_account_type:
      - Local: plain username          e.g. "jsmith"
      - AD:    DOMAIN\username         e.g. "CONTOSO\jsmith"
      - Entra: UPN                     e.g. "jsmith@contoso.com"
...
.NOTES
    ...
    Version     0.2
    Updated     Updated CSV schema (username, source_account_type); added Local/AD/Entra source support - JRR
...
$RunbookGUIDs = @("00000000-0000-0000-0000-000000000000")
...
$PspServerUrl = "https://psp.example.com/Agent"
$PspPsk       = "YOUR_PSK_HERE"

# Target identity type:
#   Are we migrating to Entra or AD?
...
$TargetIdentityType = "AD"  # Entra | AD
...
#>
    ...
    $requiredHeaders = @("computer_name","username","source_account_type","target_upn","target_identity")
...
function Get-SourceUserSid {
<#
.SYNOPSIS
    Returns the SID for a source user account by enumerating the ProfileList registry.

.DESCRIPTION
    Resolves the SID for Local, AD, or Entra accounts entirely from the registry,
    requiring no DC connectivity or NTAccount translation.

    ProfileList path:
      HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList

    Each subkey name IS the SID. ProfileImagePath is used to match the username.

    Matching logic per account type:
      - Local:  folder leaf must equal Username (case-insensitive).
                SID must NOT start with S-1-12-1- (excludes Entra).
      - AD:     Username must be DOMAIN\sam format.
                Tries folder leaf == 'sam.DOMAIN' first (Windows disambiguates conflicts this way).
                Falls back to folder leaf == 'sam' if not found (no local conflict existed).
                SID must NOT start with S-1-12-1-.
      - Entra:  Username must be UPN format (user@domain.com).
                Folder leaf must equal the part before '@'.
                SID must start with S-1-12-1-.

.PARAMETER Username
    - Local:  plain username       e.g. "jamie"
    - AD:     DOMAIN\username      e.g. "CONTOSO\jamie"
    - Entra:  UPN                  e.g. "jamie@contoso.com"

.PARAMETER AccountType
    One of: Local, AD, Entra.

.OUTPUTS
    String SID.

.NOTES
    - Throws if no matching profile is found.
    - Warns if multiple profiles match (uses first).
    - Has this user ever logged into this machine? A profile must exist.
#>
    param(
        [Parameter(Mandatory=$true)][string]$Username,
        [Parameter(Mandatory=$true)][ValidateSet("Local","AD","Entra")][string]$AccountType
    )

    $profileListPath = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList"
    $profileEntries  = Get-ChildItem -Path $profileListPath

    $foundSids = @()

    foreach ($profileEntry in $profileEntries) {
        $sid         = $profileEntry.PSChildName
        $profilePath = (Get-ItemProperty -Path $profileEntry.PSPath -ErrorAction SilentlyContinue).ProfileImagePath

        if ([string]::IsNullOrWhiteSpace($profilePath)) { continue }

        $folderLeaf = Split-Path $profilePath -Leaf
        $isEntraSid = $sid.StartsWith("S-1-12-1-")

        switch ($AccountType) {
            "Local" {
                if (-not $isEntraSid -and $folderLeaf -ieq $Username) {
                    $foundSids += $sid
                }
            }
            "AD" {
                if ($Username -notmatch '^(.+)\\(.+)$') {
                    throw ("AD username must be in DOMAIN\username format. Got: '" + $Username + "'")
                }
                $domain = $Matches[1]
                $sam    = $Matches[2]

                # Windows appends .DOMAIN when a local account with the same name exists.
                # Try the disambiguated form first, fall back to plain sam.
                $primaryFolder  = $sam + "." + $domain
                $fallbackFolder = $sam

                if (-not $isEntraSid -and ($folderLeaf -ieq $primaryFolder -or $folderLeaf -ieq $fallbackFolder)) {
                    $foundSids += $sid
                }
            }
            "Entra" {
                if ($Username -notmatch '^(.+)@(.+)$') {
                    throw ("Entra username must be in UPN format (user@domain.com). Got: '" + $Username + "'")
                }
                $upnPrefix = $Matches[1]

                if ($isEntraSid -and $folderLeaf -ieq $upnPrefix) {
                    $foundSids += $sid
                }
            }
        }
    ...
    if ($foundSids.Count -eq 0) {
        throw ("No profile found in registry for " + $AccountType + " account '" + $Username + "'. Has this user logged into this machine?")
    }

    if ($foundSids.Count -gt 1) {
        Write-Warn ("Multiple profiles matched '" + $Username + "' (" + $AccountType + "). Using the first: " + $foundSids[0])
    }
...
#   2) Load migration inputs from CSV for THIS computer.
#   3) Resolve source user SID (Local/AD/Entra) via ProfileList registry.
#   4) Resolve target SID (AD or Entra).
...
try {
    ...
    # The CSV is the "database" that maps this computer to a source user and target identity.
    ...
    #   - username:           source account on this machine (format depends on source_account_type)
    #   - source_account_type: Local | AD | Entra
    ...
    # Extract source username and account type.
    $sourceUsername = $row.username
    if ([string]::IsNullOrWhiteSpace($sourceUsername)) {
        throw ("CSV username is blank for computer '" + $computerName + "'.")
    }

    $sourceAccountType = $row.source_account_type
    if ([string]::IsNullOrWhiteSpace($sourceAccountType)) {
        throw ("CSV source_account_type is blank for computer '" + $computerName + "'.")
    }
    Write-Info ("Source account: '" + $sourceUsername + "' (" + $sourceAccountType + ")")
    ...
    # Resolve the source SID via ProfileList registry lookup.
    # This works for Local, AD, and Entra accounts without requiring DC connectivity.
    # This SID is the "from" side of the translation mapping.
    $localSid = Get-SourceUserSid -Username $sourceUsername -AccountType $sourceAccountType
    Assert-SidFormat -Sid $localSid
    Write-Info ("Source user '" + $sourceUsername + "' SID: " + $localSid)
